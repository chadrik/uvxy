...
# An excerpt of `uv generate-shell-completion zsh` from uv 0.9.30.
#
# The `uv tool run` section below is a copy, and it holds every line
# that the real script holds. The two decoy sections are shortened.
# They prove that the parser reads neither the `uv run` section nor
# the `uvx` section. A test asserts that this excerpt and the whole
# script produce the same table.
...
'--link-mode=[The method to use when installing packages from the global cache]:LINK_MODE:((clone\:"Clone (i.e., copy-on-write or reflink) packages from the wheel into the \`site-packages\` directory"
